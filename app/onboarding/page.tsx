    ...
    return (
        <div className="flex items-center justify-center gap-6 text-sm mt-6">
            <div className="flex items-center gap-1.5 text-slate-400 whitespace-nowrap">
                <span className="w-6 h-6 rounded-full border border-slate-200 text-xs flex items-center justify-center shrink-0">
                    1
                </span>
                Cuenta creada
            </div>
            <div className="w-8 h-px bg-slate-200 shrink-0 hidden lg:block" />
            <div
                className={`flex items-center gap-1.5 font-medium whitespace-nowrap ${step === 1 ? "text-indigo-500" : "text-slate-400"}`}
            >
                <span
                    className={`w-6 h-6 rounded-full text-xs flex items-center justify-center border shrink-0 ${step === 1 ? "bg-indigo-50 border-indigo-200" : "border-slate-200"}`}
                >
                    2
                </span>
                Tu negocio
            ...
            <div className="w-8 h-px bg-slate-200 shrink-0 hidden lg:block" />
            <div
                className={`flex items-center gap-1.5 font-medium whitespace-nowrap ${step === 2 ? "text-indigo-500" : "text-slate-400"}`}
            >
                <span
                    className={`w-6 h-6 rounded-full text-xs flex items-center justify-center border shrink-0 ${step === 2 ? "bg-indigo-50 border-indigo-200" : "border-slate-200"}`}
                >
                    3
                </span>
                Primer servicio
...
export default function OnboardingPage() {
    ...
    function validateStep1() {
        ...
        if (!bizName.trim() || bizName.trim().length < 3)
    ...
    function validateStep2() {
        const e: Record<string, string> = {};
        if (!svcName.trim() || svcName.trim().length < 2)
        ...
        if (svcStart >= svcEnd)
            e.end = "El horario de fin debe ser posterior al inicio";
        setStep2Errors(e);
        return Object.keys(e).length === 0;
    }
    ...
    async function handleStep2Submit(e: React.FormEvent) {
        ...
        try {
            ...
            setError(
                "Ya existe un negocio con ese nombre. Probá con un nombre diferente.",
            );
            setLoading(false);
        }
    }
