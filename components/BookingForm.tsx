...
export function BookingForm({ slot, businessSlug, business }: BookingFormProps) {
    ...
    return (
        <div className="flex flex-col gap-6">
            <Card className="p-4">
                <p className="text-slate-400 text-sm mb-1">Turno seleccionado</p>
                <p className="text-slate-800 font-semibold">{slot.service ?? 'Servicio'}</p>
                <p className="text-slate-600 text-sm">{slot.startTime} – {slot.endTime}</p>
                <p className="text-indigo-500 font-semibold mt-1">${slot.price.toLocaleString('es-AR')}</p>
            </Card>
            ...
            <div className="flex flex-col gap-4">
                <Input label="Nombre completo" placeholder="Juan García"
                    ...
                    error={errors.name} />
                <Input label="Email" type="email" placeholder="juan@mail.com"
                    value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })}
                    error={errors.email} />
                <Input label="Teléfono" type="tel" placeholder="+54 9 11 1234-5678"
            ...
            {!usesMP && (
                <p className="text-center text-slate-400 text-xs">Pago seguro simulado con MercadoPago</p>
            )}
        </div>
    );
}
